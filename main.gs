...
function Install(){
  // ClearCache();
  ScriptApp.newTrigger('main').timeBased().everyMinutes(UPDATE_IN_MINUTES).create();
}
...
function main() {
  ...
  for (const time of togglTimesEntries) {
    ...
    if(endTime !== null) {
      const existEvent = CalendarApp.getDefaultCalendar().getEvents(startTime, endTime);
      if(existEvent.length === 0) {
        const event = calendar.createEvent(time.description, startTime, endTime);
